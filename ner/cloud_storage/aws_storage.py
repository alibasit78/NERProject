...
class SimpleStorageService:
    ...
    def upload_directory_to_s3(self, directory_path, bucket_name, s3_folder=""):
        """
        Uploads a directory recursively to an S3 bucket.

        Args:
            directory_path (str): Path to the local directory.
            bucket_name (str): The name of the S3 bucket.
            s3_folder (str): The S3 folder path where the files should be uploaded.
        """
        ...
        for root, dirs, files in os.walk(directory_path):
            for file in files:
                ...
                # Create the relative S3 path by stripping the local directory path from the root
                relative_path = os.path.relpath(local_file_path, directory_path)

                # If you want to upload files to a specific folder in S3, append the folder to the S3 path
